...
$$
\begin{aligned}
\text{DiagonalGaussPdf}(\vec{x}) &= \prod_{i=1}^{n} \text{GaussianPdf}(x_i; \mu_i, \sigma)
\end{aligned}
$$
...
- `sigma`: ([optional]{.smallcaps}) number giving $\sigma$, shared by every dimension. Defaults to `1.0`
- `dimensions`: array of structs, each assigning one categorical value to the corresponding dimension's mean:
    ...
    - `mean`: name of the mean $\mu_i$, [must]{.smallcaps} resolve to either a data set axis, parameter or function.
...
- `dimensions`: array of structs, each assigning one category value to the corresponding dimension's mean:
    - `category`: string,  [must]{.smallcaps} match one of the category values assumed by `category_index`
    - `mean`: name of the mean $\lambda_i$, [must]{.smallcaps} resolve to either a data set axis, parameter or function.
