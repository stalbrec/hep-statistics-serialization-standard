...
#### Diagonal distribution {#sec:diag-distribution label="sec:diagonal-distribution"}
...
A Diagonal distribution is a struct defined by the following components:
...
- `default_parameters`: ([optional]{.smallcaps}) struct assigning each named parameter $p_{i,j}$ of the function $f$ to a number or name, for dimensions that do not specify it on their own
- `dimensions`: array of structs, each assigning one categorical value to the corresponding dimensions parameters:
  - `category`: string, [must]{.smallcaps} match one of the category values assumed by `category_index`
  - `parameters`: struct assigning each named parameter $p_{i,j}$ of the function $f$ to a number or name. If at least one parameter in at least one dimension is ommited here the top-level component `default_parameters` becomes [required]{.smallcaps} and a default [must]{.smallcaps} be declared for that parameter.
